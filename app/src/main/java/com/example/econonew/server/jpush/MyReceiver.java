...
public class MyReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (JPushInterface.ACTION_REGISTRATION_ID.equals(intent.getAction())) {
            ...
            Log.d(TAG, "[MyReceiver] 接收到推送下来的自定义消息: " + pushMsg);
            JSONObject ob = JsonCast.getJsonObject(pushMsg);
            String type = JsonCast.getString(ob, "type");
            String title = JsonCast.getString(ob, "title");
            ArrayList<MsgItemEntity> list = new ArrayList<>();
            list.add(new MsgItemEntity(title, null, null, null, null));
            MainMessage messageManager = MainMessage.getInstance(type);//根据type获取相应的消息管理的对象
            if (messageManager != null) {
                messageManager.setMessage(list, true, true);
            }
            Log.v("pushMsg", pushMsg);
            processCustomMessage(context, bundle);

        } else if (JPushInterface.ACTION_NOTIFICATION_RECEIVED.equals(intent.getAction())) {
            Log.d(TAG, "[MyReceiver] 接收到推送下来的通知");
            @SuppressWarnings("unused")
            PushNotificationBuilder notice_builer = new PushNotificationBuilder() {

                @Override
                public Notification a(String arg0, Map<String, String> arg1) {
                    return null;
                }
            ...
            String pushAlert = bundle.getString(JPushInterface.EXTRA_ALERT);
            Log.v("alert", pushAlert);
