...
public class DB_Information extends SQLiteOpenHelper {
	...
	private void createSelfTable(SQLiteDatabase db) {
		...
		for (int i = 0; i < Constant.selfDataTableNames.length; i++) {
			String sqlStr = "CREATE TABLE "
					...
					+ "code text DEFAULT NONE," + "channel_id integer" + ")";
			db.execSQL(sqlStr);
		}
	}
	...
	public ArrayList<MsgItemEntity> getLoveChannelMsgs(String channelName) {
		...
		for (query.moveToFirst(); query.moveToNext();) {
			...
			String titleImageUrl = query.getString(query
					.getColumnIndex("image_title"));
			String contentUrl = query.getString(query
					.getColumnIndex("contentUrl"));
			MsgItemEntity entity = new MsgItemEntity(msgTitle, null,
					contentUrl, null, titleImageUrl);
			entity.setLove(true);
			loveMsgs.add(entity);
		}
		db.close();
	...
	public void saveAllMsg(String tableName, List<MsgItemEntity> list) {
		...
		for (MsgItemEntity entity : list) {
			if (!dataHasSave(tableName, entity)) {
				ContentValues values = new ContentValues();
				values.put("context", entity.getMsgContent() == null ? ""
						: entity.getMsgContent());
				values.put(
						"title",
						entity.getMsgTitle() == null ? "" : entity
								.getMsgTitle());
				values.put(
						"image_title",
						entity.getImageTitleUrl() == null ? "" : entity
								.getImageTitleUrl());
			...
			if (count > 0) {
				for (cursor.moveToFirst(); !cursor.isAfterLast(); cursor
					...
					String channelType = cursor.getString(cursor
							.getColumnIndex("type"));
					String channelArrtibute = cursor.getString(cursor
							.getColumnIndex("attribute"));
					String code = cursor.getString(cursor
							.getColumnIndex("code"));
					int channelId = cursor.getInt(cursor
